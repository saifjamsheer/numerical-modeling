function gameMedium(x1,y1,x2,y2,m1,m2,planet,grav,pres,temp)
% Function that takes in inputs x1 (drone 1 x-coordinate), y1 (drone 1
% y-coordinate), x2 (drone 2 x-coordinate), y2 (drone 2 y-coordinate), m1
% (drone mass), m2 (projectile mass), planet (planet the game is taking
% place on), grav (gravity on aforementioned planet), pres (pressure on
% aforementioned planet) and temp (temperature on aforementioned planet) in
% order to launch the medium version of an interactive game.
...
pause(3);
fprintf(['Your job is to figure out the necessary launch angle to intercept two drones that have been terrorizing',' ', planet,'.\n']);
pause(3);
fprintf('No pressure.\n');
pause(2);
...
if ~(strcmpi(skip,'s'))
    ...
    pause(6);
    fprintf(['3. The two drones are at [',num2str(x1),',',num2str(y1),'] and [',num2str(x2),',',num2str(y2),'] respectively ([x,y]).\n']);
    pause(6);
    ...
    pause(8);
    fprintf(['8. You have 7 attempts to take down both drones. If you fail,',' ',planet,' ','will be destroyed.\n']);
    fprintf('\n');
    pause(6);
...
fprintf('So here''s the beautiful projectile launcher, and those are the two forking drones in the distance!\n');
pause(3);
...
c = scatter(x2,y2,70,'mx'); % Plotting the second drone location

legend([a,c,b],'Launcher Position','Drone 1 Location', 'Drone 2 Location'); % Creating a legend
...
% failed attempts 
while (hit1 ~= 1 || hit2 ~= 1) && attempts < 7
    ...
    leftAttempts = num2str(7-attempts);
